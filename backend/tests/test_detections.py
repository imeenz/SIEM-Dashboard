...
async def test_get_detections(authenticated_client):
...
async def test_get_detection_by_id(authenticated_client):
    ingest_response = await authenticated_client.post(
    ...
    detections_response = await authenticated_client.get(
        "/api/v1/detections"
    )
...
async def test_detection_not_found(authenticated_client):
    response = await authenticated_client.get(
        "/api/v1/detections/999999"
    )
...
async def test_detection_pagination(authenticated_client):
    response = await authenticated_client.get(
        "/api/v1/detections?skip=0&limit=10"
    )
